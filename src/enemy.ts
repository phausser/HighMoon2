...
export function initializeEnemyShip(width: number, height: number): void {
  if (state.enemyShip.x === 0 && state.enemyShip.y === 0) {
    state.enemyShip.x = ENEMY_MARGIN_LEFT;
    state.enemyShip.y = height / 2;
  }
...
export function respawnEnemyShip(now: number): void {
  ...
  state.enemyShip.y = randomBetween(
    state.enemyShip.length / 2,
    canvas.height - state.enemyShip.length / 2,
  );
  state.enemyShip.angle = 0;
  state.enemyShip.energy = ENEMY_MAX_ENERGY;
  state.enemyShip.active = true;
  state.enemyShip.entering = true;
  state.enemyShip.lastFiredAt = now;
  state.enemyShip.nextMoveAt = -1;
  state.enemyShip.targetY = state.enemyShip.y;
  state.enemyShip.respawnAt = -1;
...
function findEnemyAimAngle(now: number): number {
  const base = Math.atan2(
    state.ship.y - state.enemyShip.y,
    state.ship.x - state.enemyShip.x,
  );
  ...
  for (const yOff of ENEMY_STILL_SIM_Y_OFFSETS) {
    ...
    for (const aOff of ENEMY_STILL_SIM_ANGLE_OFFSETS) {
      if (simulateHitsPlayer(state.enemyShip.x, testY, testAngle + aOff)) {
        found = true;
        break;
      }
    }
    if (found) {
      state.enemyShip.targetY = testY;
      state.enemyShip.nextMoveAt = now + ENEMY_STAY_MAX_MS;
      break;
    }
  }
...
export function spawnEnemyProjectile(now: number): void {
  ...
  const nose = state.enemyShip.length / 2;
  state.enemyProjectiles.push({
    x: state.enemyShip.x + dx * nose,
    y: state.enemyShip.y + dy * nose,
...
export function updateEnemyShip(deltaSeconds: number, now: number): void {
  ...
  if (!state.enemyShip.active) {
    if (state.enemyShip.respawnAt >= 0 && now >= state.enemyShip.respawnAt) {
  ...
  if (state.enemyShip.entering) {
    ...
    if (state.enemyShip.x >= ENEMY_MARGIN_LEFT) {
      state.enemyShip.x = ENEMY_MARGIN_LEFT;
      state.enemyShip.entering = false;
  ...
  if (now >= state.enemyShip.nextMoveAt) {
    ...
    const h = state.enemyShip.length / 2;
    state.enemyShip.targetY = clamp(
      state.enemyShip.y + dir * randomBetween(ENEMY_MOVE_MIN_PX, ENEMY_MOVE_MAX_PX),
      h,
      canvas.height - h,
    );
    state.enemyShip.nextMoveAt = now + randomBetween(ENEMY_STAY_MIN_MS, ENEMY_STAY_MAX_MS);
  ...
  state.enemyShip.angle = Math.atan2(
    state.ship.y - state.enemyShip.y,
    state.ship.x - state.enemyShip.x,
  );
...
export function drawEnemyShip(): void {
  ...
  const zl = state.enemyShip.length * state.zoomLevel;
  const zw = state.enemyShip.width * state.zoomLevel;
  context.save();
  context.translate(zx, zy);
  context.rotate(state.enemyShip.angle);
...
export function updateEnemyProjectiles(deltaSeconds: number, now: number): void {
  ...
  for (const p of state.enemyProjectiles) {
    if (state.circles.some((c) => isProjectileCollidingWithAsteroid(p.x, p.y, c))) {
      spawnParticles(p.x, p.y, calculateProjectileEnergy(p, now), '255, 68, 68', now);
      continue;
    }
    if (now >= p.canHitShipAfter &&
